function calcularRV() {
    ...
    let qtddu = parseInt(document.getElementById("qtddu").value) || 0;
    let migracao = parseFloat(document.getElementById("migracao").value) || 0;
    let detrator = parseInt(document.getElementById("detrator").value) || 0;
    ...
    if (clientes < 40 || prod < 0.3 || tpv < 5000) {
        ...
        if (prod >= 0.3 && prod <= 0.39) valorComissao = 300;
        else if (prod >= 0.4) valorComissao = 450;
    } else if (tpv >= 7500) {
        if (prod >= 0.3 && prod <= 0.39) valorComissao = 450;
        else if (prod >= 0.4) valorComissao = 900;
    ...
    let valorBase = 0;
    if (valorComissao >= 100 && valorComissao < 300) {
        valorBase = 200;
    } else if (valorComissao >= 300 && valorComissao < 450) {
        valorBase = 400;
    ...
    let percT3Smart = prod > 0 ? (t3_smart / (prod*8*qtddu)) * 100 : 0;
    let percCNPJ = prod > 0 ? (cnpj / (prod*8*qtddu)) * 100 : 0;
    ...
    // Cálculo da penalidade por qtd de clientes abaixo de R$1000
    let penalidade = detrator / clientes;
    let punicao = 0;
    if (penalidade <= 0.1) {
        punicao = -0.05 * valorBase; 
    } else if (penalidade > 0.1 && penalidade <= 0.2) {
        punicao = -0.1 * valorBase;
    } else if (penalidade > 0.2 && penalidade <= 0.3) {
        punicao = -0.15 * valorBase;
    } else if (penalidade > 0.3 && penalidade <= 0.4) {
        punicao = -0.2 * valorBase;
    } else if (penalidade > 0.4 && penalidade <= 0.5) {
        punicao = -0.25 * valorBase;
    }
